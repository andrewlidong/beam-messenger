...
import { SocketManager } from "./user_socket.js";
...
window.initChat = function(roomId, token, userId, username) {
  // Global registry for live chat instances (one per room)
  if (!window._chatInstances) window._chatInstances = {};

  // If an instance for this room already exists, just return it
  if (window._chatInstances[roomId]) {
    return window._chatInstances[roomId];
  }

  // Obtain (or create) the singleton Phoenix Socket via SocketManager
  ...
  // Track this instance so we don't create duplicates
  window._chatInstances[roomId] = chat;
...
class Chat {
  ...
  destroy() {
    ...
    if (window._chatInstances && window._chatInstances[this.roomId] === this) {
      delete window._chatInstances[this.roomId];
    }
  }
}
